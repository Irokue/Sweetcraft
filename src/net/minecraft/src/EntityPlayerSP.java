...
public class EntityPlayerSP extends EntityPlayer
{
    ...
    public EntityPlayerSP(Minecraft par1Minecraft, World par2World, Session par3Session, int par4)
    {
        ...
        {
        	try{	
	        	URL url = new URL((new StringBuilder()).append("http://launcher.sweetcraft.fr/skins/").append(par3Session.username.toLowerCase()).append(".png").toString());
				HttpURLConnection huc = (HttpURLConnection) url.openConnection();
				huc.setRequestMethod ("GET");  //OR  huc.setRequestMethod ("HEAD"); 
				huc.setRequestProperty("User-Agent", "Mozilla/5.0 (Windows; U; Windows NT 6.0; en-US; rv:1.9.1.2) Gecko/20090729 Firefox/3.5.2 (.NET CLR 3.5.30729)");
				huc.connect () ; 
				if(huc.getResponseCode() == HttpURLConnection.HTTP_OK){
					skinUrl = (new StringBuilder()).append("http://launcher.sweetcraft.fr/skins/").append(par3Session.username.toLowerCase()).append(".png").toString();
				}else{
